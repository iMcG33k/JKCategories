...
//  SWFMDBHelper
//
//  Created by iMcG33k on 15/3/27.
//  Copyright (c) 2015年 iMcG33k. All rights reserved.
//

#import "SWFMDBHelper.h"
#import "FMDB.h"

#define SW_DOCUMENT_DIRECTORY NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).firstObject

#define SW_APP_NAME    ([[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleName"])

#define SW_FMDB_PATH [NSString stringWithFormat:@"%@/%@.db", SW_DOCUMENT_DIRECTORY, SW_APP_NAME]

@interface SWFMDBHelper ()
...
@implementation SWFMDBHelper
SingletonM(SWFMDBHelper)
...
+ (void)initialize{
    ...
    //取出实例
    SWFMDBHelper *helper=[SWFMDBHelper sharedSWFMDBHelper];
  
    //创建队列
    FMDatabaseQueue *queue =[FMDatabaseQueue databaseQueueWithPath:SW_FMDB_PATH];
